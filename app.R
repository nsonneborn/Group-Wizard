...
ui <- fluidPage(
  ...
  # Application title and introductory text
  ...
  p("CSV of preferences must have the following headers:"),
  # 
  # blank.matrix <- rbind(rep("", times=5), rep("",times=5))
  # colnames(blank.matrix) <- c("Camper", "Choice 1", "Choice 2", "Choice 3", "Choice 4")
  # choices_template <- as.data.frame(blank.matrix)
  # choices_template
  #
  pre("Camper    Choice 1    Choice 2    Choice 3    Choice 4"),
  p("CSV of capacities must have the following headers:"),
  pre("chore    capacity"),
  p("To see example CSV files and more detailed instructions, visit",
    a(href= "https://github.com/nsonneborn/Group-Wizard", "GitHub")),
  hr(),
